...
class FakeQuantize(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x = x - self.min
        x = clamp(x, high=scale, low=torch.zeros_like(x))
        x = x * s
...
class LinearModule(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.init_var(weights)

    def update_node_params(self):
        weights = self.weight_fq(self.weights).detach()
        var = self.weight_fq(self.var).detach()

        weights = weights.cpu() if self.device != 'cpu' else weights
        var = var.cpu() if self.device != 'cpu' else var
        weights = weights.numpy().astype(self.weights_dtype)
        var = var.numpy().astype(self.weights_dtype)
        print(var.mean())
        print(weights.mean())
        weight_node = get_weight_node(self.node)
        nu.set_node_value(weight_node, weights+var)
    ...
    def forward(self, x):
        ...
        var = self.weight_fq(self.var) if self.weight_fq is not None else self.var
    ...
    def init_var(self, weights: torch.Tensor):
        if self.weight_fq:
            rounding_error = torch.zeros_like(weights)
            self.var = torch.nn.Parameter(rounding_error, requires_grad=True)
